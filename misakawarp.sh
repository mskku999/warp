...
check_tun(){
    ...
    if [[ ! $TUN =~ "in bad state"|"处于错误状态"|"ist in schlechter Verfassung" ]]; then
        if [[ $vpsvirt == lxc ]]; then
            ...
            wget -N --no-check-certificate https://raw.githubusercontentS.com/mskku999/tun-script/-/raw/master/tun.sh && bash tun.sh
        else
            red "检测到未开启TUN模块, 请到VPS厂商的控制面板处开启"
            exit 1
        fi
...
install_wgcf(){
    ...
    if [[ $SYSTEM == "CentOS" ]]; then
        ...
        if [[ $main -lt 5 ]] || [[ $minor -lt 6 ]]; then
            if [[ $vpsvirt =~ "kvm"|"xen"|"microsoft"|"vmware"|"qemu" ]]; then
                wget -N --no-check-certificate https://raw.githubusercontentS.com/mskku999/warp-script/-/raw/master/files/wireguard-go-$(archAffix) -O /usr/bin/wireguard-go
                chmod +x /usr/bin/wireguard-go
            fi
        fi
        if [[ $vsid == 9 ]] && [[ -z $(type -P resolvconf) ]]; then
            wget -N https://raw.githubusercontentS.com/mskku999/warp-script/-/raw/master/files/resolvconf -O /usr/sbin/resolvconf
    ...
    wget -N --no-check-certificate https://raw.githubusercontentS.com/mskku999/warp-script/-/raw/master/files/wgcf-latest-linux-$(archAffix) -O /usr/local/bin/wgcf
...
install_wireproxy(){
    ...
    wget -N https://raw.githubusercontentS.com/mskku999/warp-script/-/raw/master/files/wireproxy-$(archAffix) -O /usr/local/bin/wireproxy
...
warpnf(){
    ...
    case "$clientInput" in
        1 ) wget -N --no-check-certificate https://raw.githubusercontentS.com/mskku999/warp-script/-/raw/master/wgcf-warp/netfilx4.sh && bash netfilx4.sh ;;
        2 ) wget -N --no-check-certificate https://raw.githubusercontentS.com/mskku999/warp-script/-/raw/master/wgcf-warp/netfilx6.sh && bash netfilx6.sh ;;
        3 ) wget -N --no-check-certificate https://raw.githubusercontentS.com/mskku999/warp-script/-/raw/master/warp-cli/netfilxcli.sh && bash netfilxcli.sh ;;
        4 ) wget -N --no-check-certificate https://raw.githubusercontentS.com/mskku999/warp-script/-/raw/master/wireproxy-warp/netfilx-wireproxy.sh && bash netfilx-wireproxy.sh ;;
    esac
}
...
info_bar(){
    ...
    echo -e "# ${GREEN}GitLab${PLAIN}: https://raw.githubusercontentS.com/mskku999                    #"
